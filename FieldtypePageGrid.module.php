...
class FieldtypePageGrid extends FieldtypeMulti implements Module, ConfigurableModule {
  ...
  public static function getModuleInfo() {
    ...
    return array(
      ...
      'installs' => array('InputfieldPageGrid', 'ProcessPageGrid', 'PageFrontEdit', 'ProcessPageClone', 'ProcessPagesExportImport'),
  ...
  public function clone($event) {
    ...
    if ($itemsParent->id) {
      ...
      foreach ($cloneItemsParent->find('') as $clone) {
        ...
        if ($this->isAutoGeneratedTitle($clone)) {
  ...
  /**
   * Returns true if the page's title was auto-generated (contains the hyphenated template name).
   *
   * @param Page $page
   * @return bool
   */
  public function isAutoGeneratedTitle(Page $page): bool {
    $hyphenatedName = str_replace('_', '-', $page->template->name);
    return strpos($page->title, $hyphenatedName) !== false;
  }
  ...
  public function publishDraft($pageId) {
    ...
    foreach ($draft->find('include=all') as $child) {
        ...
        if ($this->isAutoGeneratedTitle($child)) {
